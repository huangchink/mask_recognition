...
while(cap.isOpened()):
  ...
  # 偵測人臉
    face_rects, scores, idx = detector.run(frame, 0,-0.3)
...
# 取出所有偵測的結果
  ...
  # 以方框標示偵測的人臉
        ...
        if guess[i]==0:
            text='mask'
            cv2.rectangle(frame, (x1, y1+10), (x2, y2), (0, 255,0), 4, cv2.LINE_AA)
            cv2.putText(frame, text, (x1, y1), cv2.FONT_HERSHEY_DUPLEX,0.7, (255, 255, 255), 1, cv2.LINE_AA)
        else:
            text='no mask'
            cv2.rectangle(frame, (x1, y1+10), (x2, y2), (0, 0, 255), 4, cv2.LINE_AA)
