...
try: 
    print("載入模型 mtcnn_model.h5")
    model = load_model('mtcnn_model.h5')
except:
    ...
    model.save('mtcnn_model.h5')
    print("mtcnn_model.h5 模型儲存完畢!")
    model.save_weights("mtcnn_model.weight")
    print("mtcnn_model.weight 模型參數儲存完畢!")
...
for i, d in enumerate(face_rects):
  ...
  # 以方框標示偵測的人臉
    ...
    if guess[i]==0:
        ...
        cv2.putText(img, text, (x-2, y), cv2.FONT_HERSHEY_DUPLEX,
          0.4, (255, 255, 255), 1, cv2.LINE_AA)
    else:
        text='no mask'
        cv2.rectangle(img,(x-2,y),(x+w,y+h),(0,0,255),2)
